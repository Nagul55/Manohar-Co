...
document.addEventListener('DOMContentLoaded', () => {
    ...
    initGridScan();
    initProductTabs();
    // Delay Beams init to ensure layout is computed
    setTimeout(() => {
        initBeams();
    }, 100);
});
...
function initCardNav() {
    ...
    const calculateHeight = () => {
        ...
        const gap = 8;
        const padding = 16;
        const calculatedHeight = topBarHeight + (cardHeight * cardCount) + (gap * (cardCount - 1)) + padding;
...
function initProductTabs() {
    ...
    // Tab click handler
    const tabOrder = ['wires', 'lights', 'switches', 'fans', 'plumbing', 'conduit'];
